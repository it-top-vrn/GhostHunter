...
string GiveKey(bool id);//прини мает значение идендификации комнаты, (был ли раньше тут игрок) и на основе этого, либо выдает ключ либо нет. Так же дополняет текст находкой.
//Итемы (вещи)
...
void Disclaimer() {
	...
	cout << "\t\t\t\t\t";
	cout << "\t\t\t\t\t";
	cout << "(c) Старинин Андрей, Питецкий Владимир, Кристина Аристова" << endl;
	cout << endl;
	cout << "\t\t\t\t";
	cout << "		>>> Press Enter! <<<\n\n\n\n";
	system("pause");
...
void ChangeTargetFight(int mobHels_1, int mobDMG_1, int mobHels_2, int mobDMG_2) {
	...
	string variationOfAttack[7]{ "Наброситься на", "Истезать с жестокостью", "Раскретиковать", "Выбесить", "Соблазнить", "Накормить не свежей шавермой", "Гопануть" };
...
void Room_A3() {
	...
	if (idlvl[3])
	{
		...
		cout << "\t|                                                                                                 |" << endl;
		cout << "\t| Но к вашему сожалению, вас одолели бесы... Яблоко пленило весь ваш разум, вы чувствуете, что    |" << endl;
		...
		cout << "\t| радугой вкусов, вы не можете больше сдерживаться, и эхом от стен комнаты раздается хруст.       |" << endl;
		cout << "\t|                                                                                                 |" << endl;
		cout << "\t|                                                                                                 |" << endl;
		cout << "\t| Ваша несдержанность " << playerName << ", в этот раз принесла вам пользу!" << endl;
		...
		//cout << "123456789_123456789_123456789_123456789_123456789_123456789_123456789_123456789_123456789_123|5678|__|"; - 'это разметка текста.
		cout << "\t|                                                                                                 |" << endl << "\t|";
		cout << "\tВорвавшись в эту комнату ты находишь, ты едва не подскальзываешься на огрызке!            |" << endl;
		cout << "\t|                                                                                                 |" << endl;
		cout << "\t| Но к вашему счастью, с грацией кошки в приземлились на все 4 лапы. Очевидно, что здесь          |" << endl;
		cout << "\t| кто-то раскидал яблочные огрызки. Интересно, кто?                                               |" << endl;
		cout << "\t|                                                                                                 |" << endl;
		cout << "\t| Мы обезательно отыщим нарушителя позже, а пока надо решить куда отправится дальше!              |" << endl;
...
rechoice:
	if (mobId[5])
	{
		...
		cout << "\t|                                                                                                  |" << endl;
		cout << "\t|Хватит стоять как истокан! Пора убираться от сюда! Куда отправимся?                               |" << endl;
		cout << "\t|                                                                                                  |" << endl;
		TextWindow();
		Interface();
		idlvl[5] = false;
	...
	void Room_A7() {
		...
		if (mobId[7])
		{
			...
			//cout << "123456789_123456789_123456789_123456789_123456789_123456789_123456789_123456789_123456789_123|5678|__|"; - 'это разметка текста.
			cout << "\t|                                                                                                  |" << endl << "\t|";
			cout << "\tВорвавшись в эту комнату ты натыкаешься на " << doobleName[0] << " и его лучшего друга - "<< doobleName[1]<< "! Очень жаль " << playerName << "." << endl;
